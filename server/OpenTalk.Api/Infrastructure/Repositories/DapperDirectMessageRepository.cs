...
public class DapperDirectMessageRepository(IDbConnectionFactory db) : IDirectMessageRepository
{
    ...
    public async Task<IEnumerable<DirectMessage>> GetRecentAsync(long userId, long peerId, int limit = 50, long? beforeId = null)
    {
        string sql;
        object param;
        if (beforeId.HasValue)
        {
            sql = @"SELECT id, sender_id AS SenderId, receiver_id AS ReceiverId, type, content, file_url AS FileUrl, created_at AS CreatedAt
                    FROM direct_messages
                    WHERE ((sender_id = @UserId AND receiver_id = @PeerId)
                        OR (sender_id = @PeerId AND receiver_id = @UserId))
                      AND id < @BeforeId
                    ORDER BY id DESC LIMIT @Limit";
            param = new { UserId = userId, PeerId = peerId, BeforeId = beforeId.Value, Limit = limit };
        }
        else
        {
            sql = @"SELECT id, sender_id AS SenderId, receiver_id AS ReceiverId, type, content, file_url AS FileUrl, created_at AS CreatedAt
                    ...
                    WHERE (sender_id = @UserId AND receiver_id = @PeerId)
                       OR (sender_id = @PeerId AND receiver_id = @UserId)
                    ORDER BY id DESC LIMIT @Limit";
            param = new { UserId = userId, PeerId = peerId, Limit = limit };
        }
        using var conn = db.Create();
        return await conn.QueryAsync<DirectMessage>(sql, param);
    }
}
